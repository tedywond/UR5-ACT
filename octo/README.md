...
Running evaluation
```
$ PYTHONPATH=$LMN_CODE_DIR:/opt/act:`pwd`/examples python -m examples.03_eval_finetuned --finetuned_path=/project/models
```
...
- `batch`
  - `task`:
    - `language_instruction`: array of 'pick up the cube and hand it over'
    - `pad_mask_dict`: `{'language_instruction: array([True, ..., True])}`
  - `observations`
    - `image_primary`: np array (batch_size, 1, 256, 256, 3)
    - `proprio`: np array (batch_size, 1, 14)
    - `timestep`: np array (batch_size, 1)
    - `pad_mask_dict`
    - `pad_mask`
  - `action`: numpy array (batch_size, 50, 14)
  - `dataset_name`
  - `absolute_action_mask`

How to run??
```
$ lmn run elm -- PYTHONPATH=/opt/act python -m examples.02_finetune_new_observation_action --pretrained_path=hf://rail-berkeley/octo-small --data_dir=/project/aloha_sim_dataset/ --save_dir foo --batch_size 64
```

The original Dataset (zip file) is in RLDS format. Whatever that means.
It seems there are only `50` trajectories. (Seriously??)
Each trajectory is roughly ~300 steps (?)
Then that's converted to `dlimp` DLataset class that is a wrapper around `tf.data.Dataset`

Some wandb logs:
https://wandb.ai/takuma-yoneda/octo?workspace=user-takuma-yoneda

## Some details we noticed
- In Octo, the dataset they used have *relative action* (i.e., delta in end-effector)
> we first removed all Open-X datasets that contain no image streams and those that do not use delta end-effector control.
  - They use absolute action for *gripper state*, (i.e., open vs close), tho. They said relative action in this didn't work
  - Yet, somehow in their finetuning example (ALOHA simulated env), they use `absolute_action_mask=[True] * 14`.
- A model config (i.e., `OctoModel.load_pretrained('hf://blah/blah').config`) contains the following keys:
  - `model.observation_tokenizers`
    - defines a list of observation keys that the model should expect
  - `model.heads.action`
    - defines action info (such as action_dim)

# Doing Finetuning and Inference on UR5

The 20 trajectories on ur5 arm is at

https://drive.google.com/drive/folders/1197C7fm8uMNBQT5HgGMagjMXd6LxWqPo?usp=sharing

dirctory `0215` contains the end-effector poses and joint states. The images directory contains the `images` from the arm and side camera.

8. To finetune octo with the dataset you collected, run
```
python packages/octo/octo/data/create_dataset.py
```
save the dataset as a tensorflow dataset and the run
```
python packages/octo/scripts/finetune_ur5_new.py
```

9. To do inference on the UR5 arm, run the first 4 steps. Set
```
ur5env.run('Put the yellow block in the wooden box')
```
in the gym.py file and run
```
rosrun ur5_octo gym.py
```